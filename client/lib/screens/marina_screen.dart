...
class _MarinaScreenState extends State<MarinaScreen> {
  ...
  void _scheduleStealCreditHintLoad() {
    ...
    Future<void> load() async {
      final info = await TheftCooldownCreditService.load('boat_theft');
  ...
  Future<void> _repairInstantWithCredits(
    ...
    if (!boat.repairInProgress) {
      final started = await provider.repairVehicle(boat.id);
      if (!mounted) return;
      if (!started) {
        showTopRightFromSnackBar(
          context,
    ...
    await _finishRepairWithCredits(boat);
  }
  ...
  Widget _buildBoatCardItem(
    ...
    return VehicleCard(
      ...
      onSell: () => _sellVehicle(provider, boat),
      onScrap: () => _scrapVehicle(provider, boat),
                        ...
                        if (isSmall) ...[
                          Tooltip(
                            ...
                            child: TheftCooldownStealControl(
                              ...
                              onCreditRedeem: () => _redeemMarinaStealCooldown(
                                vehicleProvider,
                                authProvider,
                              ),
