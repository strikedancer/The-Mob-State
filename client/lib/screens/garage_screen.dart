...
class _GarageScreenState extends State<GarageScreen> {
  ...
  Future<void> _loadSelectedVehicle() async {
    try {
      ...
      print('[GarageScreen] Error loading selected vehicle: $e');
    }
  }
  ...
  List<dynamic> _getSortedVehicles(VehicleProvider provider) {
    var vehicles = _isMotorTab
        ? provider.inventory
              ...
              .toList()
        : provider.inventory.where((v) => v.vehicleType == 'car').toList();
            ...
            children: [
              Row(
                ...
                children: [
                  Row(
                    children: [
                      const Icon(Icons.garage, color: goldColor, size: 20),
              ...
              ClipRRect(
                ...
                child: LinearProgressIndicator(
                  value: garageStatus.getUsagePercentage(),
                  backgroundColor: const Color(0xFF3A3A3A),
                  valueColor: AlwaysStoppedAnimation<Color>(
                    garageStatus.isFull() ? Colors.red.shade700 : goldColor,
                  ),
                  minHeight: 6,
                ),
              ),
  ...
  Widget _buildVehicleGrid(VehicleProvider provider) {
    ...
    final cars = _getSortedVehicles(provider);
  ...
  Future<void> _stealVehicle(
    ...
    final stealType = _isMotorTab ? 'motorcycle' : 'car';
    final success = await provider.stealVehicle(currentCountry, stealType);
    ...
    if (success) {
      ...
      if (stolenVehicle != null) {
        await _showStolenVehicleDialog(stolenVehicle, xpGained: xpGained);
      } else {
        showTopRightFromSnackBar(
          context,
          SnackBar(
  ...
  Future<void> _showStolenVehicleDialog(
    VehicleInventoryItem vehicle, {
    int xpGained = 0,
  }) async {
    ...
    await showDialog<void>(
      ...
      builder: (context) {
        ...
        return AlertDialog(
          title: Text(
            l10n.stolenVehicleTitle(
              vehicle.vehicleType == 'motorcycle'
                  ? _tr('motor', 'motorcycle')
                  : l10n.vehicleTypeCar,
            ),
          ),
          content: SizedBox(
  ...
  Future<void> _upgradeGarage(
    ...
    if (success) {
      showTopRightFromSnackBar(
        context,
        SnackBar(content: Text(AppLocalizations.of(context)!.garageUpgraded)),
      );
    } else {
      showTopRightFromSnackBar(
        context,
        SnackBar(
          content: Text(
            provider.error ?? AppLocalizations.of(context)!.garageUpgradeFailed,
          ),
        ),
      );
    }
  ...
  Future<void> _showAvailableVehicleCatalog(
    ...
    await provider.fetchStealableCatalog(
      category: _isMotorTab ? 'motorcycle' : 'road',
    );
    if (!mounted) return;

    final cars = provider.availableVehicles.where((vehicle) {
      final category = (vehicle.vehicleCategory ?? vehicle.type ?? '')
          .toLowerCase();
      if (_isMotorTab) {
  ...
  Future<void> _selectForCrimes(dynamic vehicle) async {
    try {
      ...
      // Check if vehicle is in same country as player
      ...
      if (response.statusCode == 200) {
        ...
        final data = jsonDecode(response.body);
        final errorMessage =
            data['params']?['message'] ?? l10n.failedSelectVehicle;
        showTopRightFromSnackBar(
          context,
          SnackBar(content: Text(errorMessage), backgroundColor: Colors.red),
        );
