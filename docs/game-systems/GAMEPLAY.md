...
- **Domeinen:** het spel draait op `themobstate.com` (canonical) én `themobstate.nl` (zelfde app, API blijft `api.themobstate.com`). De **Almanak** staat op `https://wiki.themobstate.com` (catalogus + originele beelden, alle UI-talen). Transactionele mail (verificatie / wachtwoord-reset) komt van **`noreply@themobstate.nl`**. Als de VPS-IP op een blocklist staat, gaat die mail via Resend, niet vanaf het server-IP.
