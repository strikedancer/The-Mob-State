...
- **Marktplaats (P2P)**: naast **voertuigen** kunnen spelers **gedragen gereedschap** (tools) van speler tot speler verkopen via dezelfde tab; API `GET /market/unified` combineert voertuigen met item-advertenties. Andere categorieën (drugs, crypto, event-items) volgen in latere uitbreidingen — zie `docs/module-protocols/player-marketplace.md`.
