...
- **Berichten / postvak:** een badge of push “nieuw bericht” hoort bij bestaande threads. Spelerchats blijven gegroepeerd. Elke The Mob State-melding (badge, uitbetaling, order) is een **eigen inbox-regel**, geen lange gedeelde systeemthread. De inbox mag niet leeg lijken terwijl die berichten er wél zijn; bij een laadfout zie je retry, niet “Nog geen berichten”.
...
- Op **telefoon en tablet** (niet als de site al als PWA openstaat) toont de landing en het dashboard een banner **Zet op beginscherm**. Android Chrome opent het install-venster; iPhone/iPad toont Delen → Zet op beginscherm. Daarna opnieuw via Instellingen. Zie Help → Instellingen.
...
- **Kopen/verkopen**: live prijzen en **rugzakvoorraad**; elektronica-verkoop hangt af van **conditie**. Winst = verkoopprijs − inkoopprijs (`purchasePrice` op inventory). Hoeveelheid typen of **Max** tikken (lang −/+ = 10). Gewone reizen en hangar-vluchten kunnen rugzak-handelswaren confisqueren.
...
- **Google Sign-In (web):** zelfde flow met **Doorgaan met Google** zodra Client ID + Secret op de server staan. Alleen `openid email profile`. Nieuwe Google-spelers kiezen username, personage en voorwaarden. Een geverifieerd Google-e-mailadres koppelt aan het bestaande account. Zie `docs/module-protocols/google.md`.
