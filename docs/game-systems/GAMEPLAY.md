...
- **Productiematerialen** liggen per land in `production_materials.country` (depot), in een huis/loods (`material:` in `property_drug_storage`) of in de rugzak (`country = _carried_`). Kopen → huidig-land depot; plaatsen in een pand is optioneel om voorraad op te bouwen. Productie in land X gebruikt depot, dan pandvoorraad in X, dan rugzak. Meenemen → expliciet transfer naar rugzak (slots); travel kan rugzak-materiaal afpakken/aanhouding verhogen; landdepots en huisvoorraad blijven veilig, magazijnvoorraad niet bij aanhouding in dat land. Details: `docs/module-protocols/drugs.md`.
