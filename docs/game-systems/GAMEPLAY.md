...
- **Marktplaats (P2P)**: naast **voertuigen** (inclusief motoren en boten als voertuig-inventory) kunnen spelers **gedragen gereedschap** (tools) aan elkaar verkopen op dezelfde tab; **`GET /market/unified`** combineert voertuigen met `itemListings`. Verkopen gebeurt met contant geld; eigen advertentie kopen is geblokkeerd. **Drugs-, crypto- en event-item** P2P vereist aparte serverregels en staat op de roadmap in `docs/module-protocols/player-marketplace.md` (nog niet in de live API).
