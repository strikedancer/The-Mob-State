...
- Het dashboard gebruikt een noir/gold game-stijl met duidelijke panelen voor navigatie en status. Zijbalk en hamburger-menu zijn gegroepeerd en doorzoekbaar. **Timers lopen in de browser** (events, jail, cooldowns, footer-stippen). Home telt die seconden af zonder het hele overzicht elke seconde opnieuw te bouwen. De server-tick houdt de wereld bij (inkomen, heat, nightclub-sales); die is geen UI-klok. Sync met de server gebeurt bij **eerste load, pagina-wissel of na een actie** — geen 10/30-seconden-polls alleen om resterende tijd te zetten. Home-statistieken komen uit één parallelle `dashboard-stats`-ronde. Event-badges tonen seconden onder 24 uur. Trage `dashboard-stats`-antwoorden mogen een lokaal aftellende timer niet terugzetten.
...
- Elke spelerspagina heeft rechtsboven dezelfde gouden `i` als Don/Races; die opent de uitgebreide uitleg van die pagina (dezelfde teksten als Help & Uitleg). In **Hoe het werkt** en **Tips** staan de regels als losse bullets met ruimte ertussen, niet als één dichte lap tekst. Don en Midnight Races houden hun eigen foto-gids.
...
- Rangtitels (Soldaat, Peetvader, Don, …) zijn hetzelfde op dashboard en publiek profiel. **Peetvader** is pas vanaf rang 60; rond rang 25 is de titel **Soldaat**.
