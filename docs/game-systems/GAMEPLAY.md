...
- **Custom portretten (selfie→gangster)** in dezelfde avatar-kieslijst: tijdens generatie toont de client een **wachtdialoog** (niet weg te tikken); per tegel: **download** (links, PNG opslaan) en **verwijderen** (rechts). Je kiest een **stijl** (o.a. noir, casual, pak, avondglamour); de server gebruikt je **accountgeslacht** (registratie) in de AI-prompt. Staff kan overtredende portretten in het adminpanel wissen (zie `docs/module-protocols/player-portraits.md`).
