...
- NPC claim of a vacant factory uses `updateMany` where `ownerId IS NULL` plus MariaDB 1020 retries, so concurrent `listFactories` / ensure calls do not throw or double-assign.
