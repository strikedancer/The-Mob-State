...
- Web dashboard navigation is sidebar-first: add or change module navigation via the sidebar source (`_buildWebMenuItems` + `_WebSection` content switch) and not only via the legacy tile grid. Sidebar and hamburger menu are grouped (Acties / Wereld / Sociaal / Economie / Empire / Assets / Meer) and have a search field. **VIP** and **Credits** sit as standalone items directly under Dashboard (not inside Economie).
...
- Premium & Credits is a first-class dashboard destination when premium purchases or wallet actions are exposed to players; wire **VIP** and **Credits** under Dashboard in the sidebar and keep direct route entry (`/premium`) working for payment return flows. The store uses compact purchase rows (not giant image tiles). Cooldown/time-skip credit sinks stay on the action screens, not on this page. In-game cash packs are real-money one-time offers.
