...
- Elke spelerssectie (web-content én mobiele push-routes) toont een gouden `i` met de uitgebreide pagina-uitleg uit Help & Uitleg (`GamePageInfoHost`). Don, Midnight Races en Help & Uitleg zelf zijn uitgezonderd (eigen gids of het help-scherm). Op mobiel dashboard-home staat de `i` in de AppBar.
