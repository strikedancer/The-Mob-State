...
- Web dashboard navigation is sidebar-first: add or change module navigation via the sidebar source (`_buildWebMenuItems` + `_WebSection` content switch) and not only via the legacy tile grid. Sidebar and hamburger menu are grouped (Acties / Wereld / Sociaal / Economie / Empire / Assets / Meer) and have a search field. **Winkel**, **Almanak** (external wiki via `AlmanacLauncher`) and **Support** sit as standalone items directly under Dashboard, aligned with Dashboard (not inside Economie or Meer). VIP and Credits are tabs on that one Shop page. Do not put Help back in Meer; keep `help_content.dart` for the gold `i` page-info popups.
- **Early-rank menu:** until rank 5, grey and lock late systems (vault, hitlist, aviation, territory, casino, crypto, stocks, smuggling, drugs, nightclub, properties, don, races, prostitution, RLD, ammo factory, tune shop). Keep crimes, jobs, vehicle heist, events, court, travel, crew, friends, support, world chat, bank, black market, premium, inventory, training, school, Almanak, hospital, prison and achievements open. Locked taps show `menuUnlocksAtRank`.
...
- While jailed, opening Zwarte Markt shows the jail overlay (same pattern as jobs/crimes). Training hub stays reachable; only the shooting range is locked.
- Premium Shop (**Winkel**) is a first-class dashboard destination when premium purchases or wallet actions are exposed to players; wire it under Dashboard in the sidebar (same alignment as Dashboard) and keep direct route entry (`/premium`) working for payment return flows. The store uses compact purchase rows with VIP and Credits tabs (not giant image tiles). Cooldown/time-skip credit sinks stay on the action screens, not on this page. In-game cash packs are real-money one-time offers.
...
- Web-chrome is een operations-console (`dashboard_chrome.dart`): gecentreerd logo in de topbar (desktop min. 60px / max. 100px; op mobiel kleiner ~36–48px zodat de HUD-ruimte overblijft), platte zijbalk met gouden selectiebalk, HUD-statusrij (cash/rang/HP/wanted/FBI/land/credits/VIP) en homepanelen met sectietitels. Onder de tablet-breakpoint (~900px) stapelt de statusbalk in **twee rijen** (geld/rang/HP/VIP, daarna wanted/FBI/land/credits). Ongelezen berichten zitten op de avatar-badge, niet in de HUD. Credits-tik opent Winkel op de Credits-tab; VIP-tik opent Winkel op de VIP-tab. Winkel staat onder Dashboard, gelijk uitgelijnd. VIP toont resterende tijd (of Actief zonder einddatum, of Nee). Wanted en FBI zijn allebei 0–100 en tonen `n%` (zelfde notatie; niet het oude 5-sterrenplafond). Geen `FittedBox` in HUD-cellen (Flutter web kan daarop vastlopen). HUD-cijfers (cash/credits/wanted/FBI/HP/rang) komen uit de actie-response via `AuthProvider.updatePlayerStats` (`PlayerHudSnapshot` + `Selector`); geen volle `GET /player` en geen remount van Misdaden/Banen na elke tik. VIP-resttijd tikt lokaal via `_hudClock`. Geen gameplay-wijziging.
- Home-timeouts bevatten `crew_mission` wanneer de crew een afgeronde missie-cooldown heeft (`GET /player/dashboard-stats` → `cooldowns.crew_mission`).
- Home-timeouts bevatten `launder` terwijl een witwas-job `processing` is (`cooldowns.launder` via `getLaunderRemainingSeconds`).
...
- Home-statistieken tonen **owned fleet**-tellingen uit `vehicle_inventory`: auto’s (`vehicleThieves`), motoren (`motorcycleThieves`) en boten (`boatThieves`), gelabeld als owned cars/motorcycles/boats (niet lifetime steelpogingen).
...
- Plaats de **Start-kaart** (alleen nieuwe spelers) en de compacte **Markt**-tegel **onder** de profiel/statistiekkaarten, niet als twee balken bovenin de Home-content. **Dagdoelen** staan **één keer** in de gestylede paneelkaart (progress + cash/XP), standaard ingeklapt zoals weekdoelen; tik opent de lijst. Op web bij het speler/economie-blok. Niet nog een tweede platte lijst bovenaan. Na Claim verschijnt rechtsboven wat je kreeg. De weekdoelen-minikaart mag naast/onder die kaart blijven. Featured daily mag `vehicle_theft_1` niet pushen onder rank 5.
- Dashboard toont een compacte **Markt**-tegel (aantal actieve advertenties + categorieën in de UI-taal, geen `player_tool`-codes + CTA). Geen locatie-intel. Op web opent de tegel **Zwarte Markt → Marktplaats** in de dashboard-content (`_openBlackMarket(tabMarketplace)`), niet als fullscreen-route. Recente eigen acties staan **niet** als vaste **Mijn activiteit**-lijst op Home: open **Sessie-overzicht** (rechtsboven op web, AppBar op native). Dezelfde sheet (`showSessionRecapSheet`) dekt beide. Data blijft `GET /events?limit=50` + SSE (scoped op `playerId`).
...
- **Live event rail** (rechtsonder, niet op Events / Berichten / Crew / Wereldchat): standaard één ingeklapte chip zodat avatars geen tekst of verzendknop bedekken; tik opent de foto-avatars + resterende-tijd-badge. Monthly Empire onderaan de open stack (dichtst bij de duim); tap opent dezelfde event-detailpopup als Events (maandevent: dezelfde Event Pass-lijst als Evenementen). Rode claim-cijfer alleen op het maandevent (Event Pass), nooit op week-avatars en nooit voor dashboard dag-/weekdoelen. Zie `events.md`.
- **Game-support donate-avatar** (uiterst linksonder, 8px van de schermrand — niet tegen de zijbalk; zelfde Clash-stijl als de event-rail rechts; niet op Berichten / Wereldchat): tik opent een modal met vrij eurobedrag en Doneren. Dat is een eenmalige betaling voor servers/ontwikkeling, geen winkelproduct. Op mobiel (sticky quick-nav zichtbaar) staat de avatar **boven** de footer (`bottomOffset` ≈ 72 + safe inset), zodat hij Misdaden/Stelen niet blokkeert; desktop/sidebar houdt een klein hoek-inset.
...
- Chat-events (`direct_message.*`, `global_chat.*`) komen wél via SSE maar **niet** in Sessie-overzicht.
- **Berichten / postvak:** de badge (`GET /messages/unread`) mag nooit groener zijn dan de inbox-lijst. `GET /messages/conversations` blijft één gebatchte query; bij laadfout retry, geen lege “geen berichten”-staat. Spelers kunnen alles als gelezen zetten en geselecteerde of alle threads uit hun eigen inbox halen (`hiddenForSender` / `hiddenForReceiver`); de andere speler houdt de chat.
...
- Dashboard `availableTargetsCount` telt alleen crews die klaar zijn om aangevallen te worden (min. leden én niet in war-cooldown); `canDeclare` volgt dezelfde hub-guardrails als de War Room.
- **`GET /player/dashboard-stats`** laadt de baseline in **één parallelle query-ronde** (plus één korte ronde voor crypto-prijzen / territory-leader). Geen sequentiële lifetime-counts of drie volle Vehicle Ops intelligence-payloads op Home. Vehicle Ops op het dashboard komt uit `getVehicleOpsDashboardSummaries` (gedeelde heat/profile/season/claims). Per-request `console.log` van de stats-blob hoort niet op dit pad. Jail-clock writes in `checkIfJailed` moeten MariaDB 1020 / write-conflict retries overleven zodat Home niet 500't bij gelijktijdige player-updates.
...
- Na login toont Home een wekelijkse **Discord-koppelpopup** (`maybeShowDiscordLinkPrompt`, web) ná een eventuele premium-popup, zolang Discord niet gekoppeld is en de speler niet op “Ik heb geen Discord” tikte. Daarna de eenmalige **push-popup** (`maybeShowPushEnablePrompt`). Later slaat de push-popup over; Instellingen blijft de herhaal-entrypoint voor push. Discord-link kan ook in Instellingen.
- Op telefoon/tablet (web, niet al als PWA) kan boven de sticky footer de **PWA beginscherm-banner** staan (`PwaInstallBanner`). Chrome opent het native install-sheet; iOS toont stappen. Dismiss blijft lokaal bewaard.
...
- Verifieer na een misdaad/baan dat cash, credits, gezocht en FBI in de statusbalk meebewegen zonder dat de pagina opnieuw opbouwt.
