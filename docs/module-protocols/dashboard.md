...
- Web-chrome is een operations-console (`dashboard_chrome.dart`): gecentreerd logo in de topbar (desktop min. 60px / max. 100px; op mobiel kleiner ~36–48px zodat de HUD-ruimte overblijft), platte zijbalk met gouden selectiebalk, HUD-statusrij (cash/rang/HP/wanted/FBI/land) en homepanelen met sectietitels. Onder ~720px beschikbare HUD-breedte stapelt de statusbalk in **twee rijen van drie** (geld/rang/HP, daarna wanted/FBI/land); tekst schaalt mee (`FittedBox`) i.p.v. weg te ellipsen. Geen gameplay-wijziging.
...
- Elke spelerssectie toont een gouden `i` met de uitgebreide pagina-uitleg uit Help & Uitleg. Web: geen overlay over de contentkaart — naast de hero-titel (Misdaden/Banen) of in de compacte statusbalk. Don, Midnight Races en Help & Uitleg zelf zijn uitgezonderd. Op mobiel dashboard-home staat de `i` in de AppBar.
