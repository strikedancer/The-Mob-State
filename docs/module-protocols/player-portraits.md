...
- In the avatar picker, each custom portrait tile has **high-contrast circular actions**: **download** (left, saves the PNG via browser download on web or the OS share sheet on mobile/desktop) and **remove** (right, `DELETE /settings/portraits/:id`). Icons use **bundled Font Awesome Solid** (`FontAwesomeIcons` in `fontawesome_icons.dart`) — not Material Icons — so glyphs render reliably on **Flutter web** (Material icon font can fail to paint in some builds). Hint text explains both.
