...
- `sendToPlayer` (geen devices, stale FCM tokens, send exceptions, Firebase niet klaar) mag **geen** `system.error` meer schrijven. Die gevallen blijven `console.log` / `console.warn` in de container. Admin test-push zonder devices blijft wél een System Log (`AdminTestPush`).
- Admin → System Logs heeft een **Verberg bron**-filter (`excludeSource`) naast Alle bronnen. Standaard is `NotificationService.sendToPlayer` verborgen (localStorage); zet op **Niets verbergen** of kies die bron in de include-lijst om oude rijen alsnog te zien.
...
9. Verifieer dat admin test-push zonder devices nog in System Logs staat (`AdminTestPush`), dat `NotificationService.sendToPlayer` niet meer als nieuwe `system.error` binnenkomt, en dat **Verberg bron** die oude rijen uit de lijst haalt.
