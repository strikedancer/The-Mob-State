...
- Showroom collections (`car_showroom`, `motorcycle_showroom`, `boat_harbor`) are **not** extra garage space. Displayed vehicles stay owned but are locked and do not count toward garage/marina capacity; they must disappear from garage/marina inventory immediately after place (client refreshes `VehicleProvider`). World-cap per model still counts them. Crime selection and smuggle-as-cargo ignore exhibited rows. Cross-country move remains smuggling; the vehicle must be in the showroom country before it can go on display.
- Garage **upgrade levels** are **independent per track** (`car` vs `motorcycle`) per player per country: `garage_upgrades.track` + parallel capacity math (auto: basis `garages.capacity` + car-track bonus; motor: basis 2 slots + motorcycle-track bonus, +3 per level). `POST /garage/upgrade` accepts `garageTrack`. At max level (5) per track the client hides the upgrade control. Player help (`helpTopicGarageHow` and the Vehicle Heist info popup) must say upgrades apply only to the country you are in, that car and motorcycle stay separate, and that upgrades add slots not steal chance.
...
- Player street theft is **one button per category**. The client must call `POST /vehicles/steal` with `{ vehicleType }`; the server picks the target. Do not let the client choose a model. Risk is always a **street car** (common/uncommon). Failures name that street car. After a success the server may upgrade to rare/epic/legendary with rank-scaled weights (`streetTheftRarityWeights`) and a second price-based roll. Unlocks: rare 7 (tiny until 10), epic 13, legendary **22 hard lock**. `POST /vehicles/steal/:vehicleId` stays for NPC/targeted calls and still enforces catalog `requiredRank`.
...
- Country availability lists must be robust: normalize country ids to lowercase, and avoid accidental “empty catalog” failures. For boats, an empty/missing `availableInCountries` list is treated as global availability (unless a regional blacklist event blocks it). Legacy spawn tags that are not travel countries (`monaco`→`france`, `austria`→`germany`, `united_kingdom`→`uk`) must resolve to a playable country in catalog content, steal filters, and Almanac pages.
...
- Personal garage repair/refuel/tune/sell stay keyed on `vehicleInventory.id` + owner. Vehicles deposited into crew storage leave this inventory and are managed from Crew → Storage instead. Crew-side tuning spends parts from crew parts storage, not the acting player's personal parts pool.
...
- Clear success and failure feedback for the player. After a successful theft the result popup must offer sell and scrap on the same card (with confirm), plus keep. Selling, scrapping and claiming a chop contract use the same gold/dark confirmation popup (vehicle still, payout, irreversible warning). The **Ops Contract** button also confirms first but does **not** consume a vehicle; **Claim contract** does. On compact width the sell control stacks **Verkopen / Sell** above the euro amount so the payout is never ellipsized away.
...
- Every vehicle requires at least 3 unique state variants: new (100%), dirty and damaged. Catalog and theft UI must use those files, never the shared placeholder `image` generated by `prepare_vehicle_image_placeholders.py`.
