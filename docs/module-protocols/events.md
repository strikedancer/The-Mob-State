...
- **Event Pass** (`season_pass_monthly`, Mollie one-time €7.99): **50 category goals** per `YYYY-MM` month (crimes, vehicles, smuggling, drugs, cash earned, XP). Free column = event prize; premium column = pass bonus (requires unlock). Per-category stats in `player_season_pass` (`stat_*` columns). API: `/season-pass/status`, `/season-pass/claim`. UI: scrollable goal list on Events screen; buy tile on Premium. The old 7-day `event_pass_7d` pack is deactivated.
