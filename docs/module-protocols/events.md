...
- **Event Pass** (`season_pass_monthly`, Mollie one-time €7.99): **50 category goals** per `YYYY-MM` month (crimes, vehicles, smuggling, drugs, cash earned, XP). Free column = event prize; premium column = pass bonus (requires unlock). Per-category stats in `player_season_pass` (`stat_*` columns). Progress hooks: successful crime / vehicle steal / smuggling **send** / drug **collect** (grams) via `gameEventService.recordContribution` → `addSeasonPassProgress`; money/XP also from crimes/jobs. **Not** tracked: prostitution recruit (no Event Pass category). API: `/season-pass/status`, `/season-pass/claim`. UI: scrollable goal list on Events screen; buy tile on Premium. The old 7-day `event_pass_7d` pack is deactivated.
...
- **Live event rail**: ronde category-avatars rechts op het web dashboard (verborgen op Events-sectie); tap opent een sheet met countdown + top-prijs + knop naar Events. Rode badge = aantal claimbare Event Pass-beloningen voor die categorie (money/xp orphan-claimables op de eerste avatar als er geen allround/trade-event is).
