...
- **Monthly Empire Showdown** (`monthly_empire_showdown`, category `allround`): 7-day live window every ~30 days; receives score from **all** gameplay categories via `recordContribution` (OR `allround`). Player-facing UI does **not** show a rank prize pool (1st/2nd/3rd); the monthly popup shows **Event Pass claimables at the top**, then status/score/leaderboard. Week events keep the rank pot.
...
- **Live event rail**: ronde **foto-avatars** (zelfde category-art als de Events-kaarten) rechts op het web dashboard (verborgen op Events-sectie). Tot 6 stuks: **Monthly Empire Showdown** altijd gepind (actief óf upcoming/preview), daarna de actieve weekevents. Live resterende-tijd-badge overlapt de onderrand van de avatar (`endsAt`, of `startedAt` als het maandevent nog scheduled is). Tap opent **dezelfde event-detailpopup** als op de Events-pagina (`GET /game-events/:id`: status, datums, jouw score/rank, leaderboard; weekevents ook prijzenpot; maandevent toont Event Pass-claimables bovenaan i.p.v. rang-prijzen). Rode badge rechtsboven = claimbare Event Pass-beloningen (money/xp orphans op de eerste avatar als er geen allround/trade-event is).
...
- **Overview UI**: `GET /game-events/overview` levert `active`, `upcoming` (echte `scheduled` live rows) én **`upcomingPreview`** (volgende interval-starts uit enabled schedules voor templates die nu niet live zijn). De Flutter-pagina toont category-themed kaarten met bestaande achtergrond-art, countdown, top-prijs teaser (niet op het maandevent) en in de detail-dialog: weekevents prijzenpot, maandevent Event Pass-claimables bovenaan.
