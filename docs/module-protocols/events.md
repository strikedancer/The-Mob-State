...
- **Season Pass** (`season_pass_monthly`, Mollie one-time €7.99): unlocks premium reward track for current `YYYY-MM` season. Free track for everyone. Progress = live-event contribution points (`/season-pass/status`, `/season-pass/claim`). UI on Events screen + offer tile in Premium & Credits.
