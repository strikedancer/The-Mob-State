...
- **Push (FCM) + inbox**: bij start en einde van een actief live event stuurt de server een **gelokaliseerde push** naar spelers die in **Instellingen → Spelerevents** push aan hebben staan (`push_game_events`, standaard **aan**). Wie een prijs pakt (of 1e wordt) krijgt daarnaast een **persoonlijk inboxbericht** én een gerichte push (`game_event_won` / `game_event_placed`). Inbox gaat altijd; push volgt dezelfde voorkeur. Zie `gameEventNotificationService` + `playerNotificationPreferenceService`.
