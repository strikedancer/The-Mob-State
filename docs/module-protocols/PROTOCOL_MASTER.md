...
- **Training hub (gym + schietschool):** één Flutter-scherm (`training_hub_screen.dart`), labels o.a. `trainingHub*` in ARB; help-topic **`training-hub`** via `scripts/_help_topics_extracted.json` + `apply_help_topics_l10n.mjs`. Push/inbox: alleen bij **nieuwe** server-notificaties uitbreiden (`translationService` + `playerNotificationBundlesExtra`); bestaande `cooldowns.gym` / `cooldowns.shooting_range` mogen parallel blijven.
