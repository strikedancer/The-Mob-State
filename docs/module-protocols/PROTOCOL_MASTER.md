...
- **SMTP:** `emailService` leest `SMTP_HOST` / `SMTP_PORT` / `SMTP_USER` / `SMTP_PASS` / `SMTP_FROM` / `SMTP_FROM_NAME` uit de runtime-env (`docker-compose.plesk.yml` ← `.env.plesk`). From-adres = `SMTP_FROM` of anders `SMTP_USER`. Productie-mail hoort van **`noreply@themobstate.nl`** te komen (eigen SPF/DKIM/DMARC op `.nl`); mailbox + DNS eerst aanmaken, daarna `SMTP_USER`/`SMTP_FROM`/`SMTP_PASS` zetten. Wachtwoord **nooit** in git; rotatie alleen op Plesk + `.env.plesk`.
