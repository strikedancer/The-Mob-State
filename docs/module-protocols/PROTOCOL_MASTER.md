...
- **Hoofdbalk-avatar (user menu):** de tooltip gebruikt ARB-key **`userAccountMenuTooltip`** (`dashboard_screen.dart`). Het korte label **`account`** leest in o.a. NL als **bank/zakelijke “rekening”** en is daarvoor ongeschikt als hint voor het profiel-/instellingenmenu. Menu-item **Mijn profiel** gebruikt **`myProfile`**.
...
- **SMTP:** `emailService` leest `SMTP_HOST` / `SMTP_PORT` / `SMTP_USER` / `SMTP_PASS` / `SMTP_FROM` / `SMTP_FROM_NAME` uit `.env.plesk`. Productie-From én envelope zijn **`noreply@themobstate.nl`** (Gmail accepteert die; PTR/HELO is `mail.themobstate.nl`). SMTP-login blijft **`noreply@themobstate.com`** (Plesk-alias heeft geen eigen mailbox). `smtp_address_preference=ipv4`. Auth-mails blijven lichte HTML + text. Secrets nooit in git.
...
- **E-mailverificatie-gate:** runtime key `AUTH_REQUIRE_EMAIL_VERIFICATION` (`1` = aan / default, `0` = uit). Beheer via **Admin → Config → Toegang**. Uit: login slaat `EMAIL_NOT_VERIFIED` over, registratie geeft direct een JWT en stuurt geen verificatiemail. Accounts blijven `emailVerified: false`. Aan: registratie stuurt weer mail, login eist verificatie. Geen mass-resend bij aanzetten. Live na opslaan, geen deploy nodig. Speler kan vanaf login **opnieuw een verificatiemail** vragen (`POST /auth/resend-verification` met username + wachtwoord, 90s cooldown).
...
- **Discord:** community-invite (`DISCORD_INVITE_URL`, `GET /public/community` + footer/Help/Instellingen) en Discord Sign-In (`GET /auth/discord/start` → `/login?d=ok|pending|error`, `POST /auth/discord/complete`). Scopes: `identify email guilds.join` (auto-join The Mob State guild when `DISCORD_GUILD_ID` + bot token). Secrets: `DISCORD_CLIENT_ID` / `DISCORD_CLIENT_SECRET` / `DISCORD_GUILD_ID`. Publieke patch notes via `DISCORD_UPDATES_WEBHOOK_URL` + `scripts/post_discord_update.ps1` (niet de Crew Wars-webhook). Optionele wereldchat-brug: `GLOBAL_CHAT_DISCORD_WEBHOOK_URL` / `GLOBAL_CHAT_DISCORD_BOT_TOKEN` / `GLOBAL_CHAT_DISCORD_CHANNEL_ID` (nooit `#updates`). Protocols: `discord.md`, `global-chat.md`.
...
- **Admin → Spelers:** `GET /admin/players` toont standaard **alleen echte accounts** (rijen in `npc_players` worden weggelaten). Query `includeNpcs=1` zet NPCs weer in dezelfde lijst; de UI heeft daar een vinkje voor. Spelerdetail (`GET /admin/players/:playerId/overview`) toont `lastLoginIp` / `lastLoginIpAt` (gezet bij login en bij authenticated game-requests).
...
- **Flutter web API-basis (landing + rest):** zonder `--dart-define=WEB_API_BASE_URL=...` moet een productie-build op `themobstate.com` / `themobstate.nl` (en `www`) alsnog de juiste backend bereiken — zie `client/lib/config/app_config.dart` (apex → `api.themobstate.com`; lokaal blijft `:3000` op de page host).
- **CORS (productie, cross-subdomain):** `backend/src/config/index.ts` **union** in productie altijd de Mob State-shell-origins (`https://themobstate.com`, `www`, `admin.themobstate.com`, `https://themobstate.nl`, `www.themobstate.nl`, `admin.themobstate.nl`) met `ALLOWED_ORIGINS` uit `.env`, zodat een te smalle env-lijst de browser niet blokkeert. **`backend/src/app.ts`:** registreer `cors` vóór `waitForPrisma` — anders krijgen 503/fout-responses geen `Access-Control-Allow-Origin` en meldt de browser ten onrechte alleen CORS (terwijl bv. de DB down is). Admin-login vanaf `.nl` moet CORS hebben; zonder `admin.themobstate.nl` faalt Safari-preflight en komt er geen POST (lijkt op “inloggen mislukt”, geen IP-ban).
- **Admin login logging:** `POST /admin/auth/login` mag geen wachtwoorden of volledige request bodies loggen (alleen `username` + fail/success zonder secrets). `lastLoginAt` alleen bij login (fire-and-forget), niet op elke authenticated admin-request. Login trimt spaties en vindt ook de compacte naam (`erwin h` → `erwinh`). SUPER_ADMIN mag extra admin-accounts verwijderen (`DELETE /admin/admins/:adminId`) en wachtwoorden resetten; niet het eigen account en niet de laatste actieve SUPER_ADMIN.
- **Player staff (Mod/Ops):** `players.staffRole` (`NONE`/`MOD`/`OPS`), alleen Super-admin zet dit op Players. Mod/Ops loggen op admin.themobstate.com in met hun **spelaccount**. Mod: alleen Wereldchat. Ops: Wereldchat + Tickets + Spelers (lezen, geen geld/rank/VIP/ban/config). In-game: badge + wissen/muten/reports in wereldchat. Protocol: `staff-roles.md`.
- **Territory (player + push/inbox):** het scherm `client/lib/screens/territory_screen.dart` gebruikt **alleen** `AppLocalizations` voor labels, tabs, legenda, fouten, dialogs en snackbars (`territory*` keys). Bron voor nieuwe/gewijzigde schermteksten: `scripts/territory_ui_l10n_data.json` en `node scripts/inject_territory_ui_arb.mjs` (schrijft naar `app_en.arb` / `app_nl.arb`); daarna `node scripts/merge_arb_missing_all_from_en.mjs` en in `client/` `flutter gen-l10n`. **Let op:** na merge zijn `de`/`fr`/`es`/`it`/`pl`/`pt`-waarden vaak nog **letterlijk Engels** totdat ze gevuld zijn; voor alleen `territory*`-keys: vanaf `scripts/` draaien `node translate_arb_english_fallback.mjs --langs=de,fr,es,it,pl,pt --prefix=territory` (netwerk + `google-translate-api-x`), daarna opnieuw `flutter gen-l10n`. Handmatige kwaliteit (NL/ES): overrides in `territory_ui_l10n_data.json` of module-specifieke apply-scripts. **FCM-push en systeem-inbox** bij contest-start, voorbereiding klaar (acties ontgrendeld), regio veroverd en regio verloren: `backend/src/services/territoryService.ts` leest per speler per event **één keer** `preferredLanguage` (`player.findUnique`), bouwt daarmee één `getTranslations`-bundle en stuurt push én inbox met dezelfde resolved taal (geen tweede DB-read voor inbox). **EN + NL** staan in `translationService.ts` (`notification.territoryContestStarted` / `territoryContestActiveAttacker` / `territoryContestActiveDefender` / `territoryCaptured` / `territoryLost` met `pushBody` + `inboxMessage`; afzendernaam `common.territorySystemSender`), **de, fr, es, it, pl, pt** in `backend/src/i18n/playerNotificationBundlesExtra.ts` (zelfde structuur). Nieuwe territory-notificatiecopy: type uitbreiden in `translationService` + alle bundles + aanroep in `territoryService`.
...
- **Munitiefabriek (player + API-fouten + push-label):** `client/lib/screens/ammo_factory_screen.dart` — **geen** NL/EN-splitsing voor Zwarte-markt-infoblok of bullet; gebruik `ammoFactoryBlackMarket*`, `ammoFactoryActionBlackMarket` en `ammoFactoryErr*` (mapping op `error`-codes uit `backend/src/routes/ammoFactories.ts`). Zwarte Markt-tabs zitten in `black_market_screen.dart` en gebruiken bestaande `l10n`-labels (o.a. `ammoMarket`). **Pacing (Aug 2026):** server claim-interval **20 min** + `BASE_ROUNDS_PER_TICK=3` — UI-teksten (`factorySession*`, `ammoFactoryIntro`, `ammoFactoryActionProduce`, `helpTopicAmmoFactoryHow`) en scherm-constanten moeten synchroon blijven; zie `ammo-factory.md` / `balance-economy.md`. Na nieuwe keys: `node scripts/merge_arb_missing_all_from_en.mjs`, `node translate_arb_english_fallback.mjs --langs=de,fr,es,it,pl,pt --prefix=ammoFactory`, `flutter gen-l10n`. Controleer ook marketplace-strings (`ammoStock`, `factoryProduceStatus*`, `ammoRounds*`) op verkeerde machinevertalingen (bijv. “Aktie” i.p.v. voorraad). **Push `cooldown_expired`:** indien ooit `actionType` `ammo_factory` wordt gebruikt, staat `COOLDOWN_ACTION_LABEL.ammo_factory` in `notificationService.ts` klaar (zelfde talen als allowlist).
- **Support-ticket categorieën:** `supportCategoryBug` is de softwareterm **Bug** (EN/NL/DE/IT e.d.); nooit als insect vertalen (*Beestje* / *Insekt* / *Insetto*). Zie `support-tickets.md`.
- **Prostitutie-rivaliteit:** challenge via spelersnaam of ID (`rivalUsername` / `rivalPlayerId` op `POST /rivalries/start`); UI geen numeriek-only keyboard. Zie `prostitution.md`.
- **Marktplaats P2P (Zwarte markt):** speler-tegen-speler verkoop op de tab met ARB-key **`marketplace`**. Data komt van **`GET /market/unified`** (combineert voertuigen + `itemListings`). Live kinds: `player_tool`, `drug_lot`, `crypto_lot`, `trade_good_lot` (+ voertuigen via black-market service). UI-teksten: prefix **`bmHub*`**. Help: **`helpTopicBlackMarket*`** bij gedragswijziging bijwerken. Moduleprotocol: **`player-marketplace.md`**. Productie-deploy: `scripts/vps_pull_and_build.ps1` voert `prisma migrate deploy` uit — stack-lots vereisen `quantity`/`meta` op `player_market_listings`.
...
- Bij aanpassingen of nieuwe modules is het verplicht om te controleren of `Help & Uitleg` nog klopt; als player-gedrag, flows, uitleg of terminologie verandert, moet de help-content in dezelfde wijziging worden bijgewerkt. **Help-onderwerpen in de player-client** gebruiken `helpTopic*`-keys in `app_*.arb` (bron: `scripts/_help_topics_extracted.json` + `node scripts/apply_help_topics_l10n.mjs`). Na wijziging van EN-helptekst: `app_nl.arb` (en zo nodig `app_es.arb`) handmatig of via je eigen apply-flow; voor `de`/`fr`/`it`/`pl`/`pt`: `node scripts/translate_arb_english_fallback.mjs --langs=de,fr,es,it,pl,pt --prefix=helpTopicBlackMarket --force` (prefix = camelCase key-prefix voor dat help-onderwerp), daarna `flutter gen-l10n` en `verify_arb_parity.mjs`.
...
**Material icons:** `client/assets/images/materials/<id>.png` (id uit `drugs.json`). Leonardo: `backend/scripts/generate_material_images_leonardo.py`. `vps_pull_and_build.ps1` kopieert naar `runtime/client-images/materials/` (`/images/materials/`). Shop gebruikt `WebAssetHelper` + donkere thumb-plaat.
**Don hub photos:** `client/assets/images/don/*.png` (hero, rackets, NPC sharks, officials, contracts). Deploy kopieert naar `runtime/client-images/don/` (`/images/don/`). Client: `don_screen.dart` via `WebAssetHelper` + icoon-fallback. Gouden `i` opent de geïllustreerde `donInfo*`-gids. Collect- en overige Don-timers tikken lokaal op de kaarten (`collectReadyAt`).
...
**Job catalog art:** `client/assets/images/jobs/<id>_job.png` + wiki hub `wiki/hubs/jobs.png` + `backgrounds/jobs_background.png`. Leonardo: `backend/scripts/generate_job_images_leonardo.py --force --confirm-batch YES --mirror-client-assets`. Jobs screen and wiki `/jobs/` use the dedicated wash; gym keeps `gym_bg.png`. After replacing stills, bump `kCatalogArtCacheBust` / wiki `CATALOG_ART_V` so web does not keep the old 1h-cached `Image.asset` URL.

**Jail / timeout overlay stills:** `client/assets/images/cooldown_{jail,crimes,jobs,airfield,school}.png` → `/images/cooldown_*.png`. Leonardo: `backend/scripts/generate_jail_image_leonardo.py` and `generate_cooldown_overlay_images_leonardo.py` (`--force --confirm-batch YES --mirror-client-assets`). `vps_pull_and_build.ps1` copies them to the runtime mount. Client overlays + help banners use `WebAssetHelper` + `kCooldownArtCacheBust`. Do not keep leftovers under `client/images/`.
...
6. Als de wijziging **speler-gericht** is (gameplay/UI/login, niet docs-only): post een **volledige NL+EN** changelog naar Discord `#updates` én een korte NL+EN teaser in de in-game wereldchat met `.\scripts\post_discord_update.ps1` (`-Intro`/`-IntroEn` verplicht, 4–8 bullets per taal, `-WorldChatNl`/`-WorldChatEn`; webhook `DISCORD_UPDATES_WEBHOOK_URL`). Geen titel-only koppen, commit-hashes, IP’s of spelersmails. Crew Wars-events blijven op het aparte ops-webhook. Details: `discord.md`.
...
PROTOCOL_MASTER.md (JIJ BENT HIER)
    ...
    │   ├── staff-roles.md (player Mod/Ops, scoped admin login, wereldchat-tools)
