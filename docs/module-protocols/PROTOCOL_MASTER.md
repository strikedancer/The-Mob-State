...
- **SMTP / ESP:** `emailService` stuurt via **Resend** als `RESEND_API_KEY` gezet is (`docker-compose.plesk.yml` ← `.env.plesk`); anders fallback naar `SMTP_HOST` / `SMTP_PORT` / `SMTP_USER` / `SMTP_PASS`. From-adres: `MAIL_FROM_ADDRESS` (default `noreply@themobstate.com`). Domein in Resend verifiëren (DNS records van Resend), API-key nooit in git. Het Plesk-mailboxwachtwoord van `noreply@` blijft alleen nodig zolang SMTP-fallback actief is.
