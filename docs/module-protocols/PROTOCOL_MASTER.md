...
- Referrals -> Friends, Auth, Facebook/Google login, Crimes, Jobs, Messages, Dashboard, Balance & Economy, Marketing web
...
- **Admin login logging:** `POST /admin/auth/login` mag geen wachtwoorden of volledige request bodies loggen (alleen `username` + fail/success zonder secrets). `lastLoginAt` alleen bij login (fire-and-forget), niet op elke authenticated admin-request.
