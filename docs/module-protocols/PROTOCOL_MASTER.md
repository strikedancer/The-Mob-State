...
**Jail overlay still:** `client/assets/images/cooldown_jail.png` → `/images/cooldown_jail.png`. Leonardo: `backend/scripts/generate_jail_image_leonardo.py --force --confirm-batch YES --mirror-client-assets`. `vps_pull_and_build.ps1` copies it to the runtime mount. Client jail overlay / prison help banner use `WebAssetHelper` + `kJailArtCacheBust`.
