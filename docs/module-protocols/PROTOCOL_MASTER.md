...
**Job catalog art:** `client/assets/images/jobs/<id>_job.png` + wiki hub `wiki/hubs/jobs.png`. Leonardo: `backend/scripts/generate_job_images_leonardo.py --force --confirm-batch YES --mirror-client-assets`. Dedicated `jobs_background.png` is next once Leonardo tokens are available; until then Banen keeps `gym_bg.png`.
