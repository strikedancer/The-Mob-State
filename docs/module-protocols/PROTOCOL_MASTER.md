...
- **Marketing web (gast-landing + juridisch):** `LandingScreen`, `LegalPrivacyScreen`, `LegalDigitalGoodsScreen` — alle zichtbare strings via ARB (`landing*`, `legalPrivacy*`, `legalDigitalGoods*`); hero-copy is **tekst uit ARB** (geen verplicht logo-image in de Flutter-hero; achtergrond mag wel artwork tonen). **Gasttaal:** `LocaleProvider.initGuestLocale` / `persistGuestLocale` (o.a. `guest_ui_language_code` in `SharedPreferences`); **geen** `PUT /player/language` zonder sessie; na login blijft `LocaleProvider.loadLocale()` leidend. **Publieke read-only API:** `GET /public/home` in `backend/src/routes/publicMarketing.ts` (geen auth, rate limit, alleen veilige leaderboard- en territory-topvelden). **SPA / deep links:** Express serveert `client/build/web/index.html` voor onbekende GET-routes vóór de JSON-404-handler. Flutter-paden: `/`, `/login`, `/register`, `/privacy`, `/digital-goods` (`client/lib/main.dart`). Layout- en UX-details: `marketing-web.md` + `frontend-platform.md`.
...
- **Live: 503 + “CORS” door Apache (Plesk):** als `curl -sSI https://api.themobstate.com/health` **`Server: Apache`** en **`Content-Type: text/html`** toont, bereikt de reverse proxy **geen Node** (container down, crash-loop, verkeerde upstream-poort t.o.v. Plesk-doel, firewall). De browser meldt dan op alle API-calls “No `Access-Control-Allow-Origin`” — dat is **geen Express-CORS-bug**, maar een **proxy-HTML-503 zonder CORS-headers**. Herstel eerst upstream: op de VPS `docker compose ps`, `docker compose logs backend` (zie `docker-compose.plesk.yml`: backend luistert op `127.0.0.1:${BACKEND_PORT:-3000}:3000` — Plesk moet naar **dezelfde** host-poort proxien). Pas daarna verwacht je JSON van Express (met CORS) of een 503 JSON vanuit Node als alleen Prisma faalt.
- **Health vs. Prisma-gate:** `waitForPrisma` slaat `GET /health` en `GET /health/*` over. Zo blijft liveness en de health-router (incl. details) bereikbaar als MariaDB traag start of faalt; andere routes blijven achter de DB-gate (503 tot Prisma klaar is).
