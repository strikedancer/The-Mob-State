...
- **CORS (productie, cross-subdomain):** als `ALLOWED_ORIGINS` ontbreekt of leeg is terwijl `NODE_ENV=production`, vult `backend/src/config/index.ts` een veilige default in (`https://themobstate.com`, `www`, `admin`) zodat browser-`fetch`/`http` van de Flutter-shell naar `api.themobstate.com` (o.a. `GET /public/home`) niet stil faalt. Zet voor andere domeinen altijd expliciet `ALLOWED_ORIGINS`.
