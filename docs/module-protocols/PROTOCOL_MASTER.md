...
- Voor gameplay-schermen met dynamische afbeeldingen (zoals drugs, facilities, voertuigen/black market): gebruik op web altijd `WebAssetHelper.image(...)` (asset-first met network fallback), niet alleen losse `Image.asset(...)`. Zonder helper kunnen images ontbreken in `AssetManifest` bij external-images builds en vallen kaarten terug op icon-fallbacks.
