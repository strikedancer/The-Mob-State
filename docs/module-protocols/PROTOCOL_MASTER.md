...
- **SMTP / Resend:** `emailService` stuurt via **Resend** als `RESEND_API_KEY` in `.env.plesk` staat (From blijft `SMTP_FROM`, productiewaarde **`noreply@themobstate.nl`**). Zonder die key valt hij terug op Plesk-SMTP (`SMTP_HOST` / `SMTP_PORT` / `SMTP_USER` / `SMTP_PASS`). Plesk-login blijft **`noreply@themobstate.com`** (alias `.nl` heeft geen eigen mailbox). Uitgaande Postfix-HELO/PTR moeten **`mail.themobstate.nl`** zijn (`smtp_address_preference=ipv4` zodat IPv6-PTR `mail.local` niet meespeelt). Auth-mails blijven lichte HTML + text. Secrets nooit in git.
