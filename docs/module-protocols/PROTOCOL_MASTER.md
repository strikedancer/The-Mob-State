...
- **Marktplaats P2P (Zwarte markt):** speler-tegen-speler verkoop op de tab met ARB-key **`marketplace`**. Data komt van **`GET /market/unified`** (combineert voertuigen + `itemListings`). Nieuwe UI-teksten: prefix **`bmHub*`** (o.a. `bmHubSellCarriedItem`, `bmHubNoMarketListingsTitle`). Help: **`helpTopicBlackMarket*`** bij gedragswijziging van de hub bijwerken. Moduleprotocol: **`player-marketplace.md`** (API, foutcodes, deploy/migratie, roadmap drugs/crypto/events). Productie-deploy: `scripts/vps_pull_and_build.ps1` voert `prisma migrate deploy` uit — zonder migratie bestaat `player_market_listings` niet.
...
| GAMEPLAY.md (economy / P2P-verkoop) | black-market.md, **player-marketplace.md**, trade.md (contraband vs. P2P) | Marktplaats-tab: voertuigen + carried tools; toekomstige kinds in player-marketplace roadmap |
