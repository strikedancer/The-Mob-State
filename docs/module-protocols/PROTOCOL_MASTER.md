...
**Job catalog art:** `client/assets/images/jobs/<id>_job.png` + wiki hub `wiki/hubs/jobs.png` + `backgrounds/jobs_background.png`. Leonardo: `backend/scripts/generate_job_images_leonardo.py --force --confirm-batch YES --mirror-client-assets`. Jobs screen and wiki `/jobs/` use the dedicated wash; gym keeps `gym_bg.png`.
