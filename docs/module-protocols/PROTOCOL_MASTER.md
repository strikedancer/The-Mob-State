...
- **SMTP:** `emailService` leest `SMTP_HOST` / `SMTP_PORT` / `SMTP_USER` / `SMTP_PASS` / `SMTP_FROM` / `SMTP_FROM_NAME` uit `.env.plesk`. Productie-From én SMTP-login zijn **`noreply@themobstate.com`** (dezelfde mailbox). Een From-header `@themobstate.nl` met login `@themobstate.com` faalt DMARC (`adkim=s; aspf=s`) en Gmail weigert live-mail. Postfix-HELO/PTR blijven **`mail.themobstate.nl`** (`smtp_address_preference=ipv4`). Auth-mails blijven lichte HTML + text. Secrets nooit in git.
