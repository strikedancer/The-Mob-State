...
- **Training hub (gym + schietschool):** één Flutter-scherm (`training_hub_screen.dart`), labels o.a. `trainingHub*` en op crimes **`crimeTrainingBonusStrip`** in ARB; help-topics **`training-hub`** en **`crimes`** (bonusregel) via `scripts/_help_topics_extracted.json` + `apply_help_topics_l10n.mjs`. Backend: **`GET /training/status`** (`/training`) combineert gym- en schietbaan-status voor minder round-trips. Push/inbox: alleen bij **nieuwe** server-notificaties uitbreiden (`translationService` + `playerNotificationBundlesExtra`); bestaande `cooldowns.gym` / `cooldowns.shooting_range` mogen parallel blijven.
