...
- **Alle overige actieve player-UI-talen** (`de`, `fr`, `it`, `pl`, `pt` naast `es`/`nl`/`en`) mogen in de ARB’s **niet** structureel Engels blijven: na merge van nieuwe keys uit `app_en.arb` draai `node scripts/translate_app_arb_from_en.mjs` met een doellijst, bijv. `node scripts/translate_app_arb_from_en.mjs --langs=de,fr,it,pl,pt` om **Spaans en Nederlands** (hand-curated) ongemoeid te laten, of de volledige set als je alle machinevertalingen wilt verversen. E-mail-HTML blijft (`translationService`) primair **EN + NL**; **push- en in-app notificatieteksten** voor `de`…`pt` staan in `backend/src/i18n/playerNotificationBundlesExtra.ts` en `translationService.getTranslations` — wijzigingen of nieuwe notificatietypen: daar en in `notificationService` tegelijk afhandelen. **Nieuwe EU-taal toevoegen:** nieuw `app_<code>.arb` (key-pariteit), `supported_languages` + `supportedLanguages.ts`, `flutter gen-l10n`, notificatie-bundle uitbreiden of bewust EN-laten (documenteren), en ARB-vertaling/QA-plan.
