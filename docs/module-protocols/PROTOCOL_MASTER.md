...
- **NPC live-loop:** NPCs spelen via dezelfde services als spelers (`crimeService`, `jobService`, travel, hospital, gym, school, shooting range, tools, weapons, vehicle theft, properties, bank). Start-rang en XP volgen `getXPForRank`. Geen hitlist en geen automatische crew-create. Scheduler: elke 5 min één live-cyclus per actieve NPC. **Admin simuleren N uur** schuift NPC-cooldowns/jail/training mee (MATIG 20 min/tick, GEMIDDELD 12, CONTINU 10; max 150 ticks) zodat 24 uur echte pacing is, geen 8 instant-rondes. Bank max 1× per 4 sim-uren.
