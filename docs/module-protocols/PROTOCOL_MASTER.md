...
- **SMTP:** `emailService` leest `SMTP_HOST` / `SMTP_PORT` / `SMTP_USER` / `SMTP_PASS` / `SMTP_FROM` / `SMTP_FROM_NAME` uit de runtime-env (`docker-compose.plesk.yml` ← `.env.plesk`). From-adres = `SMTP_FROM` of anders `SMTP_USER`. Productie-From is **`noreply@themobstate.nl`** (SPF/DKIM/DMARC op `.nl`); SMTP-login blijft **`noreply@themobstate.com`** omdat een Plesk-alias geen eigen mailbox heeft. Auth-mails (verificatie / wachtwoord-reset) gaan als lichte HTML + text, zonder emoji en zonder “imperium”-welkomstcopy — Gmail weigert rijke of marketingachtige auth-HTML eerder dan een korte test. Wachtwoord **nooit** in git; rotatie alleen op Plesk + `.env.plesk`.
