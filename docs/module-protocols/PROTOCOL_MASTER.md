...
- **SMTP:** `emailService` leest `SMTP_HOST` / `SMTP_PORT` / `SMTP_USER` / `SMTP_PASS` uit de runtime-env (`docker-compose.plesk.yml` ← `.env.plesk`). Het mailboxwachtwoord van `noreply@themobstate.com` **nooit** in git of source zetten; rotatie alleen op Plesk + `.env.plesk`.
