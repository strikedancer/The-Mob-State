...
- Active sentence state must show: localized crime name, arrest reason (and source crime when present), total sentence, remaining time, judge profile and action buttons.
...
- An official **amnesty** writes the same `trial.record_expunged` marker per player (`source: amnesty`) without SSE spam. After that wipe, new convictions show again. Super-admin trigger: Admin → **Rechtbank** (confirm `WIPE_ALL_RECORDS`), or `POST /admin/trial/court-record-amnesty` with `{ confirm: "WIPE_ALL_RECORDS" }`, or `node dist/scripts/runCourtRecordAmnesty.js`. Each run **force**-posts a fresh world-chat promo (`promo/court_record_wipe.png`) + Discord mirror; it does not reuse an older onceImage row.
- Players can also file a paid **expunge petition** on Court (`GET /trial/expunge-quote`, `POST /trial/expunge-petition`) while free or jailed, as long as a visible **local** record remains. Live cost/odds/cooldown/fresh-arrest window come from `courtRuntimeConfig` (`COURT_EXPUNGE_*`; code defaults match the previous hardcodes: cost `100_000 + max(0, n-1)*1_000`, clamp 8–70%, fresh-arrest −15% for the first hour, 12h cooldown). Jail-math good-conduct adds `COURT_EXPUNGE_MATH_PERCENT_PER_CORRECT` (default +1%) per `prison.math_correct` since the last matching wipe, capped at `COURT_EXPUNGE_MATH_CORRECT_CAP` (default 15). Client copy (`courtExpungeRecencyRecent` / `Warm`) must say **1 hour**, not 24 hours, so the Strafblad-wissen breakdown matches `computeExpungeRecencyModifierPercent` and `court_screen` (`hours < 1`). Success chance uses `computeExpungePetitionOdds` (record length, hours since last arrest, reputation, jail math, Don judge/commissioner/alderman). Failure deducts cash only. A petition success writes `trial.record_expunged` with `countryId` and hides only that country's file (plus legacy rows without country). Amnesty and the late-game `criminal_record_wipe` crime still wipe worldwide. Success does **not** release the player.
...
- **Localized names**: judge payload uses family name + `specialtyKey` (`violence` | `financial` | `drugs` | `white_collar` | `organized`). Client localizes the judge title and specialty. Crime titles use `crimeId` via `CrimeLocalization.nameFromId` (English `crime` / `crimeName` is fallback only). Arrest keys such as `police_arrest` / `federal_arrest` must never stay as raw IDs in NL/EN UI. Record and sentence also show `arrestReason` plus `sourceCrimeId` when `outcomeFail` JSON has them.
- **Country-scoped record**: `crime_attempts.countryId` is stamped on new jail/record rows. `GET /trial/record` returns this country's `recentCrimes` plus `fbiFile` (worldwide). Local court lists/wipes only the current country; FBI panel is view-only. Legacy null-country rows stay visible on the local file until wiped there.
...
- Verify local record lists only the current country (plus legacy rows without country), the FBI panel lists worldwide convictions, and a successful petition leaves other countries on the FBI file.
- Verify jail-math correct answers raise the wipe chance (+1% each, cap +15%) and appear on the petition breakdown.
