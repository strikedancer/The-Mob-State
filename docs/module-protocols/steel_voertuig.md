...
- Op de Vehicle Heist lane cards: bij actieve theft-cooldown opent een tik op de stelen-knop dezelfde `CooldownOverlay` (crime + `vehicle_theft` / `motorcycle_theft` / `boat_theft`) als in standalone garage/marina, zodat spelers de cooldown met credits kunnen versnellen; na een mislukte stelpoging met cooldown opent deze overlay ook automatisch.
