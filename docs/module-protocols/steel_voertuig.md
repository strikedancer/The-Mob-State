...
- Na **succesvolle diefstal** via lane/ops-kaart **én** via garage/marina toont de UI dezelfde gedeelde popup (`client/lib/widgets/stolen_vehicle_dialog.dart` → `VehicleTheftResultOverlay` / `showStolenVehicleDialog`), niet alleen een top-right toast. Snapshot `lastStolenVehicle` / XP vóór inventory-refresh. Bij succes staan **Verkopen** en **Slopen** direct in die popup (met bevestiging); **Houden** sluit zonder te verkopen. Op smalle breedte (`compactWidth`) staan Verkopen en het bedrag op aparte regels zodat het bedrag niet weg ellipsized. Zelfde `POST /vehicles/sell-stolen/:id` en `POST /vehicles/scrap/:id` als de voertuigkaarten. Foto’s in catalogus en diefstal-popup gebruiken de unieke stills 100%/vies/kapot, nooit de gedeelde placeholder `image`.
