...
- Nightclub is wél koopbaar via deze module (aankoop start het nachtclub-systeem); in de embedded Empire-shell opent **Beheer** de Nightclub-module **in de content** (zelfde patroon als showroom, geen fullscreen `Navigator.push`).
...
- **Showrooms (`car_showroom`, `motorcycle_showroom`, `boat_harbor`):** `unique_per_player` — één van elk per account wereldwijd, te koop in elk land, gebonden aan het aankoop-land. Collectie, geen tweede garage. Plaatsen: zelfde land, 100% conditie, één per model, slotcap per upgrade (8 → 20 → 40 → 80 → alle types van die categorie). Getoonde voertuigen zijn vergrendeld en tellen niet in garage/haven. Beheer: `GET/POST /properties/:id/showroom`, `.../place`, `.../remove`. In de embedded Empire-shell opent Beheer **in de content** (geen fullscreen route); vitrine/plaatsen is een responsive grid met grote vierkante voertuigfoto’s (1→2→3→4 kolommen). Overzicht toont vitrineplekken, catalogusvoortgang (`used / catalogSize`), totale collectiewaarde, zeldzaamheid per voertuig (common→legendary) en rarity-tellingen. Het **publieke profiel** spiegelt die aggregaten via `ownedProperties[].showroomStats` plus een **exhibit-lijst** (naam, rarity, image) zodat bezoekers zien welke auto/motor/boot erin staat (geen land). Politie/FBI doorzoekt de vitrine bij aanhouding in dat land (~40% per voertuig). Geen PvP-diefstal, geen waakhonden.
- **Development (v1):** permanente income-boost per eigendom via bank-spend (`developmentLevel` / `lastDevelopAt`), los van warehouse capacity upgrades. Alleen op panden met passief inkomen (magazijn, nachtclub, casino, showrooms) — niet op huis/appartement. UI toont **Ontwikkel (bank)** + korte hint: cash op zak telt niet.
...
- Property TabBar matches the Drugs hub: text-only labels (`Available` / `My properties`), `isScrollable: true`, gold indicator. No tab icons.
