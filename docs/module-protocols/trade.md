...
- Personal trade inventory: **buy/claim** credits the backpack (`inventory.country = '_carried_'`). Leftover current-country lots remain sellable here until stashed. Sell and smuggle-send debit backpack (carried first, then leftover local lots). Players **place** backpack lots into a house or warehouse (`trade:` keys); withdraw returns to `_carried_`. Commercial travel and hangar Fly can confiscate **carried** trade. `maxInventory` counts backpack **plus** property stock in that country.
