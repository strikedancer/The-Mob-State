...
- Personal trade inventory is **country-scoped** (`inventory.country`, unique `playerId + goodType + country`). Buy/claim/list credit the current (or destination) country; sell and smuggle-send debit only the warehouse in the player's current country. Players can then **place** current-country lots into a house or warehouse (`trade:` keys in `property_drug_storage`) to hold for a better price; withdraw back to `inventory` before selling. Commercial travel does **not** move or confiscate these lots. `maxInventory` counts country inventory **plus** property stock in that country.
- `GET /trade/inventory` returns `currentCountry`, `inventory` (sellable here) and `storedElsewhere` (read-only other-country lots). The handelswaren-tab pins local stock at the top.
