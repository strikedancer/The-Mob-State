...
- Bug fixes: ✅
  ...
  - Territory live timers: de regio-modal telt cooldown, contest-fasen, garnizoen, war pressure en **Bevoorraad project** per seconde af zonder de SVG-kaart elke tick opnieuw te bouwen; als een timer op nul staat, herlaadt het scherm de mapdata stil zodat aanvals-/actieknoppen ontgrendelen zonder handmatige page refresh. Na een project-bevoorrading zet de client meteen `projectContributeUntil` (server `contributeCooldownSecondsRemaining` first, anders `lastContributeAt + TERRITORY_PROJECT_CONTRIBUTE_COOLDOWN_SECONDS`, default 15m): de knop wordt grijs, toont de resterende tijd, en blijft uit tot die timer klaar is.
  ...
  - Territory Fase D (seizoen/meta): seizoen-close deelt exact-once awards (expansie/defense/war-frontline) via `territory_reward_log` + crew-bank (cash multiplier), rotating `region_event` effects met actiebonus + income-penalty, en drama-snapshot op dashboard + `/public/home` + Territory season-tab (kaarten met regiogewest-namen, countdown en event-chips; geen kale ISO/key dump)
...
14. Owned regio: start/contribute safehouse-project (HQ-gated); actief project verhoogt getoonde en uitgekeerde passieve income; building/active/damaged tonen progress- of HP-balken. Na **Bevoorraad project** staat de knop uit met een live countdown tot de contribute-cooldown (default 15m) voorbij is.
