...
- Crew overview on mobile must show the HQ still as an image (not a squeezed ListTile of HQ label + style + level). Member counts use singular/plural (`1 lid` / `2 leden`). Overview HQ caption and HQ & Upgrades building status both show the **global HQ level** (L0–L19, e.g. rural style level 2 → global **6**). Style-local progress may appear as secondary context (`landelijk 2/3`) so upgrades within a style stay clear; never show only style-local `2/3` as if that were the whole HQ level.
...
- Fresh crews must start with Crew HQ and all crew storage buildings at level 1 so bank deposits, shared storage and upgrade flows work immediately without a manual bootstrap purchase. Concurrent crew/storage GETs must upsert those starter rows (unique `crewId`) instead of racing `create`, so two members opening the same new crew do not 500. Starter set includes **Tool Storage** (`tool_storage` / Gereedschapopslag): deposit carried tools only; no personal withdraw. Starter set also includes **Parts Storage** (`parts_storage` / Onderdelenopslag): deposit personal car/motorcycle/boat parts; no personal withdraw. Existing crews that already have other bays get parts storage at the same level as their tool storage so HQ upgrade gates stay fair.
- Crew-bank cash buttons on Overview are bank verbs, never jail-bail or military-retreat wording. Dutch: **Storten** / **Opnemen** (`crewUiLabelDeposit` / `crewUiLabelWithdraw`). English stays Deposit / Withdraw. Dialog titles (`crewUiTr84` / `crewUiTr85`) must match that meaning. The Overview bank balance uses `formatCurrency` / `_money` (a real `€`), never a mojibake prefix such as `â‚¬`.
...
- Extra roles besides `leader` / `co_leader` / `member`: `consigliere` (Don crew overview, no bank withdraw) and `capo` with optional `capoCountry` (crew-bank tribute only in that country). Leader sets roles via `POST /crews/:id/members/:playerId/role`. See [don.md](don.md). Help `helpTopicCrewHow` / `helpTopicCrewTips` (Almanac `/{lang}/guide/crew/`) must name those five ranks in player language.
- **Officers:** `leader` and `co_leader` may invite/kick, approve joins, toggle recruiting, start heists, run missions/deals, sell crew vehicles, and pay crew-bank buyouts. Leader-only: set roles, withdraw from crew bank, declare wars, disband.
- **Optional income share:** members can toggle `POST /crews/:id/income-share` so a runtime % (`CREW_INCOME_SHARE_PERCENT`, default 5, max 10) of personal drug sells, ammo sells, vehicle sell/scrap, and prostitution settles goes to the crew bank; `lifetimeContribution` shows on the members list. Does not stack on mandatory vehicle-ops / wholesale crew splits.
- Shared crew storage is crew inventory, not a personal garage. Cars/motorcycles/boats/weapons/ammo/trade/parts deposit into crew and are consumed by crew smuggling, missions, deals, vehicle tuning or enemy raids; they cannot be withdrawn for personal crimes. **Ammo, drug and trade bay capacity is in tiles** (same packing as backpack/house: ammo 50 rounds/tile, drugs 100 g/tile, trade `unitsPerTile` per good); `crewBuildings.json` capacities are tile counts. Parked crew cars, motorcycles and boats can be repaired, refuelled and tuned in place (`POST /crews/:id/storage/cars|boats/:itemId/{refuel,repair,tuning}`) by any member. Repair, refuel and tune money come from the crew bank (`INSUFFICIENT_CREW_FUNDS` when the vault is short). Tune parts come from crew parts storage (`INSUFFICIENT_CREW_PARTS` when that bay is short). Selling (`.../sell`) is leader/co_leader only and credits the crew bank, blocked when cash storage is full. Personal garage repair/tune/sell endpoints must not accept crew inventory ids. Deposit copies personal tuning onto the crew row and then deletes the personal tuning record. **Drug deposit** from the Crew Storage tab uses quality lots (`DrugService` / `POST /drugs/crew-storage/deposit` with `drugType`, `quality`, `quantity`), not the trade-goods inventory path; trade deposit stays on `/crews/:id/storage/trade/deposit`. Drugs can be withdrawn or wholesaled. Weapons and ammo also feed Territory (HQ reserve plus frontline arms-cache); building level stays the cap. Crew Wars raids still steal the same HQ stacks.
...
- Member trust (`trustScore` 0–100): a crew-bank deposit of at least €10.000 awards +1 once per UTC day (anti-farm via `crew.trust_deposit_day` activity); withdrawing from the bank costs the leader −5; crew mission resolve awards +tier to contributors on success/partial and −2×tier on fail (starter if no contributions). Successful heists still award +5 (unchanged).
- Crew VIP is real-money. Every member can donate into a shared pot (`/subscriptions/checkout/crew-vip-donate`) or one member can start the monthly subscription (`/subscriptions/checkout/crew-vip`). In-game crew-bank cash never buys Crew VIP. When the pot reaches the monthly price the crew gets 30 days. Gift Crew VIP by name stays a one-time 30-day gift. Admin support can also grant/extend via `POST /admin/players/:playerId/crew-vip/grant` (Manage → Crew VIP toekennen) without creating a Mollie subscription.
...
- Verify HQ & Upgrades HQ status shows **global** level first (e.g. `Niveau 6 · landelijk 2/3`), matching Overview — not style-local alone (`2/3`) that players read as “HQ is level 2”.
...
- Verify each storage bay lists the parked/stored items (name, quantity, condition/fuel/durability) with the same small catalog thumbnails as inventory. Every vehicle has three unique stills: 100% (`imageNew`), dirty (`imageDirty`), damaged (`imageDamaged`). Never use the shared placeholder `image` file. Tapping an HQ storage building opens the same list.
- Verify tapping a stored car, motorcycle or boat opens repair/refuel/tune/sell. Money comes from the crew bank; tune parts still come from the acting player. Sell is officer-only and lands in the crew bank. Repair is timed on the crew row. Tuning survives deposit. There is still no personal withdraw.
...
- Verify Help / Almanac Crew lists leader, co-leader, consigliere, capo (one country) and member, and that Ask “crew rollen” / “consigliere” lands on that page.
