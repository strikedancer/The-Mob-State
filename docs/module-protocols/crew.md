...
- Shared crew storage is crew inventory, not a personal garage. Cars/motorcycles/boats/weapons/ammo/trade deposit into crew and are consumed by crew smuggling, missions, deals or enemy raids; they cannot be withdrawn for personal crimes. Parked crew cars, motorcycles and boats can be repaired, refuelled and tuned in place (`POST /crews/:id/storage/cars|boats/:itemId/{refuel,repair,tuning}`) by any member. Repair, refuel and tune money come from the crew bank (`INSUFFICIENT_CREW_FUNDS` when the vault is short). Tune parts still come from the acting player's parts pool. Selling (`.../sell`) is leader/co_leader only and credits the crew bank, blocked when cash storage is full. Personal garage repair/tune/sell endpoints must not accept crew inventory ids. Deposit copies personal tuning onto the crew row and then deletes the personal tuning record. Drugs can be withdrawn or wholesaled. Weapons and ammo also feed Territory (HQ reserve plus frontline arms-cache); building level stays the cap. Crew Wars raids still steal the same HQ stacks.
