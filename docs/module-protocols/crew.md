...
- Crew overview on mobile must show the HQ still as an image (not a squeezed ListTile of HQ label + style + level). Member counts use singular/plural (`1 lid` / `2 leden`). Overview HQ caption and HQ & Upgrades building status both show **style-local** level (e.g. rural `2/3`), never the HQ global level over `maxLevel` (e.g. not `6/3`).
...
- Verify HQ & Upgrades HQ status shows style-local `level/maxLevel` (e.g. rural `2/3`), matching Overview — never global level over style max (`6/3`).
