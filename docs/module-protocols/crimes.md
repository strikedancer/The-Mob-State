...
## Related APIs
- **`GET /crimes`** must stay a single batched player-context load (readiness + mastery counts + training + country-police). Never re-query `crime_attempts` or player tools **per crime** on the list — that timed out the mobile crimes screen (`errorLoadingCrimes` / connection retry). Success-chance math stays identical; only the load path is batched (`computePlayerSuccessChanceFromContext`). `attemptCrime` must pass that same computed chance into `processCrimeAttempt` as `successChanceOverride` so list % and roll stay one path.
- **Onboarding street crimes:** catalog `baseSuccessChance` is the listed starting % (no 0.385 downscale). Rank 1–5 + `minLevel` 1: most fails are `FLED_NO_LOOT` (`pettyCrimeFailJailChance` = 0.28). Vandalism and graffiti have no tool gate. Do not re-apply `crimeJailChance` on a flee.
...
- **Country police pressure (live):** soft success/arrest modifiers from shared per-country pressure — see `country-police.md`. Runtime `COUNTRY_POLICE_PRESSURE_ENABLED` (code default still 0).
...
- **Low HP does change success chance:** below 70 / 40 / 20 HP the server subtracts 4 / 8 / 12 percentage points (`crimeSuccessPenaltyFromHealth` in `healthInjury.ts`). The listed `playerSuccessChance` already includes this. Hospital is the paid reset; ticks heal slowly (`applyPassivePlayerTickBatch`: one SQL `UPDATE` for all wounded players). At 0 HP the player is in ICU (crimes blocked). Wanted decay (−1/tick) and FBI heat (INT; default 0.5 → −1 every two ticks) use the same batch.
...
- The Crimes screen must show both worn slots and, per crime card, which weapon types that crime accepts and which worn weapon will be used. Changing worn weapons happens in Inventory. The crime **car** is also chosen there (paper-doll slot). Boats stay on the Marina card; motorcycles stay on the motorcycle garage card.
