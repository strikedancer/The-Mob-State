...
- **Don hub:** foto’s onder `client/assets/images/don/` en runtime `/images/don/*` (deploy kopieert naar `runtime/client-images/don/`). Client laadt via `WebAssetHelper.image` met icoon-fallback; geen extra AppBar in de embedded Empire-shell. Gouden `i` opent een scrollbare noir/gold handleiding (`donInfo*`) met `SafeArea` en clamped afmetingen. Collect/contest/squeeze/ambt/lening/contract-timers tikken lokaal op de kaart, zonder het hele Don-scherm elke seconde te rebuilden.
- **Midnight Races:** hero onder `client/assets/images/races/` (`/images/races/*`); startveld en uitslag gebruiken catalogusauto’s via `/images/vehicles/*`. Zelfde wrap-grid als Don. Gouden `i` opent een scrollbare noir/gold handleiding (`raceInfo*`) met `SafeArea` en clamped afmetingen.
- **Drugs hub:** zelfde noir/gold foto-hero als Don/Races (`drug_environment_desktop.png` via `WebAssetHelper`) plus drie tabs (Faciliteiten / Productie / Voorraad, `TabBar isScrollable: true`). Gouden `i` in de hero opent Help & Uitleg (`helpTopicDrugs*`); geen tweede `i` in de HUD. Geen extra AppBar in de embedded Empire-shell. Tabcontent scrollt zelf.
