...
- Als je pixel-perfect “knoppen in een image” wil op web: gebruik een SVG (visual) en leg er `InkWell`/`GestureDetector` klikvlakken bovenop met `LayoutBuilder` + genormaliseerde coördinaten (viewBox → pixels). Dit voorkomt layout drift en hit-test problemen.
