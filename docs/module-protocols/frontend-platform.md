...
- **Login / registratie (`login_screen.dart`):** op brede schermen staat het formulier **rechts** uitgelijnd (`Alignment` ~0.80–0.94) zodat het niet op de linker artwork valt; de **sticky legal footer** (privacy, digitale goederen, gasttaal, copyright) is dezelfde als op de landing via `GuestLegalFooter` in `Scaffold.bottomNavigationBar`.
