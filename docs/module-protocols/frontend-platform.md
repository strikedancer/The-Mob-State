...
- Admin gebruikt hetzelfde noir/goud-thema als de player-client (`admin/src/theme/gameTheme.css`). Session bootstrap via `GET /admin/auth/me`. Ops lab: cron triggers + event-chip grants; NPC tab: activate/deactivate + simulate-all. **Chrome:** `AdminPageIntro` + KPI-strip (`adminChrome.tsx`) op alle tabs; page-header houdt de titel, geen dubbele `h1`. Runtime-tabs (Drugs, Casino, Crew Missions, Landelijke politie) groeperen keys in kaarten met eenheden, defaults en unsaved-toolbar. **Voertuigen-tab:** auto's, motoren, boten en vliegtuigen als aparte catalogi. Nieuw/vies/defect (en vliegtuig-beeld) zijn thumbs van `/images/vehicles/` of `/images/aircraft/`; klik opent een lightbox. Admin `index.html` is no-cache zodat SPA-deploys niet op een oude hashed JS-bundle blijven hangen.
