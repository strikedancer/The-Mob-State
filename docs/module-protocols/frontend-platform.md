...
- **Overige Empire-hubs met foto-hero:** Nachtclub, Eigendommen, Prostitutie (RLD-tab wisselt hero-foto/titel), standalone Red Light Districts, en Munitiefabriek delen `EmpirePageHero` + `PinnedTabBarDelegate` in `empire_page_hero.dart`. Hero scrollt weg; eventuele TabBar blijft gepind. Embedded in de dashboard-shell zonder extra AppBar. Gouden `i` in de hero, niet in de HUD.
...
- **Pagina-info (gouden `i`):** geen zwevende overlay over de contentkaart (die botste met chips/refresh). Op web: naast de paginatitel in de hero (Misdaden/Banen/Drugs/Nachtclub/Eigendommen/Prostitutie/RLD/Munitiefabriek, zoals Don) of in de compacte statusbalk voor overige secties. Mobiele fullscreen-routes houden een hoek-`i` als er geen hero-knop is. Popup hergebruikt Help & Uitleg (`helpTopic*`) in noir/gold met `SafeArea` en clamped size. Meerregelige blokken (Hoe het werkt / Tips) renderen als bullets met regelafstand. Don en Midnight Races houden hun eigen geïllustreerde `donInfo*` / `raceInfo*`-gidsen (geen tweede `i`).
