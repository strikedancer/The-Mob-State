...
- **Tabbed schermen (mobiel):** hergebruik **`AdaptiveTabBar`** (`client/lib/widgets/adaptive_tab_selector.dart`) voor sectiewissels in `AppBar.bottom` of bovenaan een `Column` — onder **720px** breedte een **dropdown** met alle sectienamen; breder een normale **TabBar**. Voeg nieuwe tab-UI’s niet als losse `TabBar` toe tenzij er een duidelijke reden is.
