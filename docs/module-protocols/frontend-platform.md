...
- **PWA beginscherm (telefoon/tablet):** `PwaInstallBanner` op landing + dashboard (boven de sticky footer). Chrome/Android: knop opent het native install-sheet via `beforeinstallprompt` (JS-bridge in `client/web/index.html`). iOS Safari kan dat niet; knop toont Delen → Zet op beginscherm. Verborgen in standalone/PWA en na dismiss (`SharedPreferences`). Opnieuw via Instellingen. Helpers: `client/lib/utils/pwa_install*.dart`. `manifest.json` `orientation` is **`any`** (niet `portrait-primary`) zodat een Android-tablet/PWA naar landscape mag draaien. Een al geïnstalleerd icoon pakt dat vaak pas na opnieuw toevoegen aan het beginscherm.
