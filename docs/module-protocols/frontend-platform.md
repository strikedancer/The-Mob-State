...
- Als je pixel-perfect “knoppen in een image” wil op web: leg `InkWell`/`GestureDetector` klikvlakken bovenop met `LayoutBuilder` + genormaliseerde coördinaten (virtuele viewBox → pixels) en render de knop-visuals met Flutter widgets. Dit voorkomt layout drift en hit-test problemen.
