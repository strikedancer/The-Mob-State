...
- **Timers in de dashboard-shell:** aftellen gebeurt **lokaal** (`DateTime.now()` / remaining−1). Geen periodieke `dashboard-stats` / `action-cooldowns` / player-refresh alleen om de klok gelijk te trekken. Server-sync bij load, `_selectWebSection` / navigatie, en na een actie (SSE of response). Footer-dots: `ValueNotifier`, geen `setState` op heel `DashboardScreen`. Polls mogen lokale remaining niet omhoog zetten; nieuwe acties (lokaal 0 → server > 0) wél. Overlay-countdowns negeren parent-rebuilds die de timer terugzetten. World-tick blijft server-side (geen UI-clock). Live koersen/nightclub-ops mogen een eigen trage poll houden; dat zijn geen cooldown-timers.
