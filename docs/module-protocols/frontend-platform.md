...
- **Pagina-info (gouden `i`):** elke spelerspagina (niet login/legal/Help) toont dezelfde gouden `i` rechtsboven via `GamePageInfoHost` / `GamePageInfoButton` (`client/lib/widgets/game_page_info.dart`). De popup hergebruikt Help & Uitleg (`helpTopic*`) in noir/gold met `SafeArea` en clamped size. Meerregelige blokken (Hoe het werkt / Tips) renderen als bullets met regelafstand, niet als één dichte `\n`-tekst. Don en Midnight Races houden hun eigen geïllustreerde `donInfo*` / `raceInfo*`-gidsen (geen tweede `i`).
