...
- wiki/ (public almanac + handbook from Help ARBs; HTML generated from bind-mounted `backend/content` and `client/lib/l10n`, shared `/images` mount on :8082)
...
- Admin gebruikt hetzelfde noir/goud-thema als de player-client (`admin/src/theme/gameTheme.css`). Session bootstrap via `GET /admin/auth/me` (timeout → login form; dashboard data loads only after `/me` succeeds so a stored token cannot stampede the API during “Sessie controleren…”). Ops lab: cron triggers + event-chip grants; NPC tab: activate/deactivate + simulate-all. **Chrome:** `AdminPageIntro` + KPI-strip (`adminChrome.tsx`) op alle tabs; page-header houdt de titel, geen dubbele `h1`. Runtime-tabs (Drugs, Casino, Crew Missions, Landelijke politie) groeperen keys in kaarten met eenheden, defaults en unsaved-toolbar. **Voertuigen-tab:** auto's, motoren, boten en vliegtuigen als aparte catalogi. Nieuw/vies/defect (en vliegtuig-beeld) zijn thumbs van `/images/vehicles/` of `/images/aircraft/`; klik opent een lightbox. Admin `index.html` is no-cache zodat SPA-deploys niet op een oude hashed JS-bundle blijven hangen. Wiki `/crimes/` en de player Crimes-kaarten delen `/images/crimes/<id>_crime.png` (noir/goud catalogusstills); hub `wiki/hubs/crimes.png` en `backgrounds/crime_background.png` horen bij dezelfde batch. Wiki `/jobs/` en Banen-kaarten delen `/images/jobs/<id>_job.png`; hub `wiki/hubs/jobs.png` en `backgrounds/jobs_background.png` (niet de sportschool-`gym_bg`).
...
- **Zwarte markt (incl. handelswaren-tab) / rugzak-shop / munitiefabriek:** zichtbare tablabels, foutteksten (bijv. niet-ingelogd + token-opslaghint) en shop-UI moeten via ARB lopen (`black_market_screen.dart`, `trade_goods_tab.dart`, `backpack_shop_screen.dart`, `ammo_factory_screen.dart`). `trade_screen.dart` is alleen een doorverwijs naar de zwarte markt.
- **PWA beginscherm (telefoon/tablet):** `PwaInstallBanner` op landing + dashboard (boven de sticky footer). Chrome/Android: knop opent het native install-sheet via `beforeinstallprompt` (JS-bridge in `client/web/index.html`). iOS Safari kan dat niet; knop toont Delen → Zet op beginscherm. Verborgen alleen in standalone/PWA. **Niet nu** is 24 uur (`pwa_install_banner_dismissed_at`); een oude permanente `pwa_install_banner_dismissed` wordt genegeerd. Accept van install verbergt alleen de huidige sessie. Touch/Android telt als tablet ook bij een brede landscape-viewport. Opnieuw via Instellingen. Helpers: `client/lib/utils/pwa_install*.dart`. `manifest.json` `orientation` is **`any`**. Een al geïnstalleerd icoon pakt dat vaak pas na opnieuw toevoegen.
- **Login / registratie (`login_screen.dart`):** op brede schermen staat het formulier **rechts** uitgelijnd (`Alignment` ~0.80–0.94) zodat het niet op de linker artwork valt; de **sticky legal footer** (privacy, **algemene voorwaarden**, digitale goederen, gasttaal, copyright) is dezelfde als op de landing via `GuestLegalFooter` in `Scaffold.bottomNavigationBar`. De **drie policy-links** in die footer openen een **scrollbare dialog** (`showGuestLegalDocumentModal` + gedeelde ARB-body in `legal_marketing_document_body.dart`), geen full-page navigatie. **Registratie** vereist een **vinkje** akkoord met de voorwaarden (`registerTerms*`-ARB); de link opent dezelfde **terms-modal**. Op het **registratieformulier** staat geen terugknop naar inloggen (wel **Account aanmaken** vanaf het inlogscherm); deep link `/login` of sluiten van de landing-modal blijft beschikbaar. **`/login` en `/register`:** `AuthProvider.isInitializing` (sessie-restore) en `isSubmitting` (formulier) zijn gescheiden — de inlogknop mag niet op de bootstrap-spinner blijven hangen. `AuthWrapper` toont alleen de volledige splash tijdens `isInitializing`, niet tijdens submit (anders verdwijnt de landing-modal). **Facebook Login (web):** knop `facebookContinue` alleen als `GET /auth/facebook/status` `loginEnabled` is; OAuth landt op `/login?fb=…` (zie `facebook.md`); Google via `/login?g=…`. De client leest die query en vervangt de adresbalk naar `/login` of `/` zodat token/pending niet zichtbaar blijven. **Google** idem via `/login?g=…`. Na het lezen van de query vervangt `replaceBrowserPath` de adresbalk (`/login` of `/`) zodat token/pending niet zichtbaar blijven. Deep links `/privacy`, `/terms`, `/digital-goods` blijven **volledige pagina’s** voor bookmarks/SEO. **Modalmodus** (`embeddedModal`): compacte kaart met sluitknop; na succes sluit de landing-dialog **direct** (`onEmbeddedAuthSuccess` + auto-pop zodra `AuthProvider.isAuthenticated`). `AuthWrapper` toont daarna het dashboard; geen extra `pushReplacementNamed` vanuit de dialog. De **taal-dropdown bij registratie** moet `LocaleProvider.persistGuestLocale` aanroepen zodat `MaterialApp.locale` en alle ARB-teksten (inclusief formulier) **direct** meeschakelen — geen aparte lokale `_selectedLanguage`-status naast de provider.
...
- Instellingen (`settings_screen.dart`) bevat veel platform/permission tekst (push status, crypto push/in-app toggles). Ook die moet via ARB keys om EU-locales volledig te ondersteunen. **Avatar-picker grid (web):** gebruik `WebAssetHelper.imageHttpFirst` voor `assets/images/avatars/<id>.png` — laadt **alleen via HTTP** (`/images/avatars/…` eerst, daarna compat-paden), **geen** `Image.asset` op web (voorkomt Flutter-engine-requests naar `assets/assets/images/…` en 404-ruis; avatars staan op de nginx **externe mount** zoals `PROTOCOL_MASTER` / `client/docker/nginx.conf`).
...
- Gebruik voor brede gameplay image-loading op web een centrale helper of route (`WebAssetHelper.image(...)` of equivalent), niet losse ad-hoc `Image.asset(...)` patterns. Job/crime catalog stills must pass `kCatalogArtCacheBust` so web uses `/images/…?v=` instead of a 1h-cached `Image.asset` URL after art replacements. Jail and action-timeout overlays use `kCooldownArtCacheBust` (`/images/cooldown_*.png`).
...
- **Speler-portretten (selfie):** gegenereerde PNG’s staan op dezelfde externe mount als andere runtime images (`runtime/client-images/player_avatars/<playerId>/…`); clients tonen ze via `activePortraitPath` → `/images/player_avatars/...` (zie `AvatarHelper`, [player-portraits.md](player-portraits.md)).
...
- **Don hub:** foto’s onder `client/assets/images/don/` en runtime `/images/don/*` (deploy kopieert naar `runtime/client-images/don/`). Client laadt via `WebAssetHelper.image` met icoon-fallback; geen extra AppBar in de embedded Empire-shell. De foto-hero zit in een `NestedScrollView`-header en **scrollt weg**; de `TabBar` blijft gepind. Gouden `i` opent een scrollbare noir/gold handleiding (`donInfo*`) met `SafeArea` en clamped afmetingen. Collect/contest/squeeze/ambt/lening/contract-timers tikken lokaal op de kaart, zonder het hele Don-scherm elke seconde te rebuilden.
- **Midnight Races:** hero onder `client/assets/images/races/` (`/images/races/*`); startveld en uitslag gebruiken catalogusauto’s via `/images/vehicles/*`. De foto-hero is het eerste item van de pagina-`ListView` en scrollt met het veld mee. Zelfde wrap-grid als Don. Gouden `i` opent een scrollbare noir/gold handleiding (`raceInfo*`) met `SafeArea` en clamped afmetingen.
- **Drugs hub:** zelfde noir/gold foto-hero als Don/Races (`drug_environment_desktop.png` via `WebAssetHelper`) plus drie tabs (Faciliteiten / Productie / Voorraad, `TabBar isScrollable: true`). Hero + heat/materialen-strip scrollen weg via `NestedScrollView`; de tabstrip blijft gepind. Gouden `i` in de hero opent Help & Uitleg (`helpTopicDrugs*`); geen tweede `i` in de HUD. Geen extra AppBar in de embedded Empire-shell. Tabcontent scrollt zelf.
- **Overige spelerpagina’s met foto-hero:** gedeelde `EmpireHubScaffold` + `EmpirePageHero` + optioneel `empireGoldTabBar` in `empire_page_hero.dart`. De foto-hero scrollt weg; tabs zijn alleen tekst, `isScrollable: true`, goud, en blijven gepind. Embedded in de dashboard-shell zonder extra AppBar. Gouden `i` in de hero, niet in de HUD. Don en Midnight Races houden hun eigen geïllustreerde hero. Help, Instellingen en dashboard-home blijven zonder deze foto-kop.
- **Dashboard chrome:** topbar met gecentreerd logo (desktop oorspronkelijk formaat, mobiel lager zodat de HUD leesbaar blijft), platte zijbalk en HUD-statusrij via `client/lib/theme/dashboard_chrome.dart`. Onder de tablet-breakpoint twee rijen van drie stats; geen FittedBox in HUD-cellen. Homepanelen gebruiken dezelfde hairline/sectietitels. Geen extra AppBar in de embedded Empire-shell.
- **Pagina-info (gouden `i`):** geen zwevende overlay over de contentkaart (die botste met chips/refresh). Op web: naast de paginatitel in de foto-hero voor bijna alle spelerssecties (`EmpireHubScaffold` / Don / Races), of in de compacte statusbalk alleen waar geen hero-`i` is (Help, Instellingen, dashboard-home). Mobiele fullscreen-routes houden een hoek-`i` als er geen hero-knop is. Popup hergebruikt Help & Uitleg (`helpTopic*`) in noir/gold met `SafeArea` en clamped size. Meerregelige blokken (Hoe het werkt / Tips) renderen als bullets met regelafstand. Don en Midnight Races houden hun eigen geïllustreerde `donInfo*` / `raceInfo*`-gidsen (geen tweede `i`).
...
- **Timers in de dashboard-shell:** aftellen gebeurt **lokaal** (`DateTime.now()` / remaining−1). Geen periodieke `dashboard-stats` / `action-cooldowns` / player-refresh alleen om de klok gelijk te trekken. Server-sync bij load, `_selectWebSection` / navigatie, en na een actie (SSE of response). Footer-dots **én Home-countdowns** (jail, action-timeouts, ops-timers, vehicle-ops chips, war-phase): `ValueNotifier`, geen `setState` op heel `DashboardScreen` / `_WebDashboardHomeContent` elke seconde. Polls mogen lokale remaining niet omhoog zetten; nieuwe acties (lokaal 0 → server > 0) wél. Overlay-countdowns negeren parent-rebuilds die de timer terugzetten. World-tick blijft server-side (geen UI-clock). Live koersen/nightclub-ops mogen een eigen trage poll houden; dat zijn geen cooldown-timers.
- **API rate limit:** `error.rate_limit` toont `evStreamErrRateLimit` (niet de ruwe key). Backend: per-JWT-emmer + `trust proxy`; zie PROTOCOL_MASTER.
...
7. Verifieer op een Android-tablet PWA (beginscherm) dat draaien naar landscape de layout meedraait; `manifest.json` `orientation` moet `any` zijn.
