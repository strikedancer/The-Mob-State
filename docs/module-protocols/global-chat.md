...
- Game → Discord webhook names use `[Ops]` / `[Mod]` when the player is staff. Discord-native staff lines get a 🛡️ / 🔨 reaction. System announcements await the webhook so a one-shot script cannot `process.exit` before the Discord copy lands. Player lines stay fire-and-forget. `#updates` is a different channel and is never ingested into the lobby.
