...
3. **Mailbox / deliverability:** From én envelope zijn **`noreply@themobstate.nl`**. SMTP-login blijft `SMTP_USER=noreply@themobstate.com` (alias heeft geen mailbox). PTR/HELO zijn `mail.themobstate.nl`; houd `smtp_address_preference=ipv4`. Gmail weigert `@themobstate.com` (DBL/reputatie) maar neemt `@themobstate.nl` aan als SPF-envelope ook `.nl` is. Auth-mails blijven eenvoudige HTML + text. Secrets nooit in git.
