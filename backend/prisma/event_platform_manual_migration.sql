...
CREATE TABLE `game_event_participant_progress` (
  ...
  `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  `updatedAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  UNIQUE INDEX `game_event_participant_progress_liveEventId_subjectType_subjectKey_key`(`liveEventId`, `subjectType`, `subjectKey`),
  INDEX `game_event_participant_progress_playerId_idx`(`playerId`),
  INDEX `game_event_participant_progress_liveEventId_rank_idx`(`liveEventId`, `rank`),
  INDEX `game_event_participant_progress_liveEventId_score_idx`(`liveEventId`, `score`),
  PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `game_event_leaderboard_snapshots` (
  ...
  `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  INDEX `game_event_leaderboard_snapshots_liveEventId_boardType_snapshotAt_idx`(`liveEventId`, `boardType`, `snapshotAt`),
  PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `game_event_reward_rules` (
  ...
  `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  `updatedAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  INDEX `game_event_reward_rules_liveEventId_isActive_sortOrder_idx`(`liveEventId`, `isActive`, `sortOrder`),
  PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `game_event_reward_claims` (
  ...
  `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  `updatedAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  INDEX `game_event_reward_claims_playerId_deliveryStatus_idx`(`playerId`, `deliveryStatus`),
  INDEX `game_event_reward_claims_liveEventId_playerId_idx`(`liveEventId`, `playerId`),
  PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

ALTER TABLE `game_event_schedules`
  ADD CONSTRAINT `game_event_schedules_templateId_fkey`
...
ALTER TABLE `game_live_events`
  ADD CONSTRAINT `game_live_events_templateId_fkey`
  FOREIGN KEY (`templateId`) REFERENCES `game_event_templates`(`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `game_live_events_createdByAdminId_fkey`
...
ALTER TABLE `game_live_event_modifiers`
  ADD CONSTRAINT `game_live_event_modifiers_liveEventId_fkey`
...
ALTER TABLE `game_event_participant_progress`
  ADD CONSTRAINT `game_event_participant_progress_liveEventId_fkey`
  FOREIGN KEY (`liveEventId`) REFERENCES `game_live_events`(`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `game_event_participant_progress_playerId_fkey`
...
ALTER TABLE `game_event_leaderboard_snapshots`
  ADD CONSTRAINT `game_event_leaderboard_snapshots_liveEventId_fkey`
  FOREIGN KEY (`liveEventId`) REFERENCES `game_live_events`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;

ALTER TABLE `game_event_reward_rules`
  ADD CONSTRAINT `game_event_reward_rules_liveEventId_fkey`
  FOREIGN KEY (`liveEventId`) REFERENCES `game_live_events`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;

ALTER TABLE `game_event_reward_claims`
  ADD CONSTRAINT `game_event_reward_claims_liveEventId_fkey`
  FOREIGN KEY (`liveEventId`) REFERENCES `game_live_events`(`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `game_event_reward_claims_rewardRuleId_fkey`
  FOREIGN KEY (`rewardRuleId`) REFERENCES `game_event_reward_rules`(`id`) ON DELETE SET NULL ON UPDATE CASCADE,
  ADD CONSTRAINT `game_event_reward_claims_playerId_fkey`
