#!/usr/bin/env python3
"""Generate the jail overlay still via Leonardo.

Outputs:
  runtime/client-images/cooldown_jail.png

Optional --mirror-client-assets copies to client/assets/images/cooldown_jail.png

Requires LEONARDO_API_KEY and --confirm-batch YES.
Use --force to overwrite the old cartoon orange-jumpsuit still.
"""
...
NEGATIVE_PROMPT = (
    ...
    "oversaturated neon, cyberpunk, sterile white cell, orange jumpsuit mascot, "
    "frame, border, collage, gore, blood, group portrait"
)

PROMPT = (
    "Cinematic dark-mafia game still, photoreal, noir amber rim light, "
    "rich gold and deep shadow, centered composition, no text. "
    "View through thick iron prison bars into a dim European holding cell. "
    "A lone man in a rumpled dark shirt sits on a metal bunk, face half in shadow, "
    "amber corridor bulb, wet concrete floor, peeling paint, cigarette smoke, "
    "quiet after-arrest mood, no readable graffiti"
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate jail overlay still via Leonardo API")
...
def main() -> None:
    ...
    out_path = output_root / "cooldown_jail.png"
    ...
    raise RuntimeError(f"Jail still generation failed after {args.attempts} attempts: {last_exc}")
