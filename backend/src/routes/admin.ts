...
import {
  getCourtRuntimeConfigView,
  updateCourtRuntimeConfig,
} from '../services/courtRuntimeConfig';
import {
...
router.get('/country-police/runtime-config', async (_req, res) => {
  try {
    const config = await countryPoliceService.getRuntimeConfigView();
    return res.json(config);
  } catch (error) {
    console.error('Admin country police runtime config error:', error);
    return res.status(500).json({ error: 'Failed to fetch country police runtime config' });
  }
});
...
router.put('/country-police/runtime-config', async (req, res) => {
  try {
    ...
    console.error('Admin country police runtime config update error:', error);
    return res.status(500).json({ error: 'Failed to update country police runtime config' });
  }
});
...
router.get('/tickets', async (req: AdminRequest, res) => {
  try {
    ...
    const tickets = await supportTicketService.listAdminTickets(status);
...
router.get('/tickets/:ticketId', async (req: AdminRequest, res) => {
  try {
    ...
    const detail = await supportTicketService.getAdminTicketDetail(ticketId);
    if (!detail) {
...
router.post('/tickets/:ticketId/reply', async (req: AdminRequest, res) => {
  try {
    ...
    const parsed = adminTicketReplySchema.safeParse(req.body);
    if (!parsed.success) {
...
router.post('/tickets/:ticketId/todos', async (req: AdminRequest, res) => {
  try {
    const adminId = req.admin?.id;
    if (!adminId) {
      return res.status(401).json({ error: 'Unauthorized' });
...
router.patch('/tickets/todos/:todoId', async (req: AdminRequest, res) => {
  try {
    const adminId = req.admin?.id;
    if (!adminId) {
      return res.status(401).json({ error: 'Unauthorized' });
...
router.get('/trial/runtime-config', async (_req, res) => {
  try {
    const config = await getCourtRuntimeConfigView();
    return res.json(config);
  } catch (error) {
    console.error('Admin court runtime config error:', error);
    return res.status(500).json({ error: 'Failed to fetch court runtime config' });
  }
});

router.put('/trial/runtime-config', async (req, res) => {
  try {
    const parsed = runtimeNumericConfigUpdatesSchema.parse(req.body ?? {});
    const updated = await updateCourtRuntimeConfig(parsed.updates);
    return res.json(updated);
  } catch (error) {
    if (error instanceof z.ZodError) {
      return res.status(400).json({ error: 'Validation failed', details: error.flatten() });
    ...
    console.error('Admin court runtime config update error:', error);
